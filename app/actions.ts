...
function buildDiscountEmail(
  ...
  const rows: [string, string][] = [
    ...
    ["Discount", `${DISCOUNT_PERCENT}% off build · code ${DISCOUNT_CODE}`],
  ];
  ...
  const html = `
    <div style="font-family:ui-sans-serif,system-ui,sans-serif;color:#12121A;max-width:640px">
      <h2 style="font-size:18px;color:#0A0E52;margin:0 0 4px">Discount claim — ${DISCOUNT_PERCENT}% off build</h2>
...
function buildClaimerEmail(claim: DiscountClaim): {
  ...
  const text = [
    ...
    "",
    `It takes ${DISCOUNT_PERCENT}% off the build price. ${DISCOUNT_TERMS}`,
    "",
  ...
  const html = `
    <div style="font-family:ui-sans-serif,system-ui,sans-serif;color:#12121A;max-width:560px;line-height:1.6">
      ...
      <p style="font-size:15px;margin:0 0 20px">It takes ${DISCOUNT_PERCENT}% off the build price. ${escapeHtml(
        DISCOUNT_TERMS
      )}</p>
